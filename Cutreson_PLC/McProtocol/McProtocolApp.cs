...
namespace Cutreson_PLC.McProtocol
{
    public abstract class McProtocolApp
    {
		...
		//<----------------------------------Code by Mr.Son------------------------------------->
		public bool WriteWords(PlcDeviceType iType, int iAddress, int iSize, int[] iData)
        {
            return WriteDeviceBlock(iType, iAddress, iSize, iData);
        }
        public bool WriteWord(PlcDeviceType iType, int iAddress, int iData)
        {
            int[] data = {iData};
			return WriteDeviceBlock(iType, iAddress, 1, data);
		}
        public int[] ReadWords(PlcDeviceType iType, int iAddress, int iSize)
        {
			byte[] byteArr = ReadDeviceBlock(iType, iAddress, iSize);
        ...
        public bool ReadBit(PlcDeviceType iType, int iAddress)
        {
            ...
            {
                return bitDevice[0] == 1 ? true : false;
			}
		}
        public bool[] ReadBits(PlcDeviceType iType, int iAddress, int iSize)
        {
            int[] bitDevice;
			bool[] outBits = new bool[iSize];
			if (GetBitDevice(iType, iAddress, iSize, out bitDevice))
            {
                for(int i = 0; i < bitDevice.Length; i++)
                {
                    if(bitDevice[i] == 1) outBits[i] = true;
                }
            }
            return outBits;
		}
